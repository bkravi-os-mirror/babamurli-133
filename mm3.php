...
<?php
  ...
  $languages = array("Thai", "Deutsch", "Italian", "Mal", "Marathi", "Nep", "Polish", "Port", "Sinhala", "Spanish", 
                      "Gujarati", "K", "Pun", "Telugu", "TamilLanka", "Odia",
                );
  ...
  $language_serverfileloc = array(
    "Thai"=>"$rootdir/01. Daily Murli/43. Thai/Mp3-Thai",
    "Deutsch"=>"$rootdir/01. Daily Murli/31. Deutsch/Mp3-Deutsch",
    "Italian"=>"$rootdir/01. Daily Murli/33. Italian/Mp3-Italiano",
    "Mal"=>"$rootdir/01. Daily Murli/06. Malayalam/03. Malayalam Murli - MP3",
    "Marathi"=>"$rootdir/01. Daily Murli/12. Marathi/04. Marathi Murli - Mp3",
    "Nep"=>"$rootdir/01. Daily Murli/30. Nepali/01. Nepali Murli - MP3",
    "Polish"=>"$rootdir/01. Daily Murli/40. Polish/MP3-Polish",
    "Port"=>"$rootdir/01. Daily Murli/41. Portuguese/MP3-Portuguese",
    "Sinhala"=>"$rootdir/01. Daily Murli/44. Sinhala/Mp3-Sinhala",
    "Spanish"=>"$rootdir/01. Daily Murli/32. Spanish/Mp3-Spanish",
    "Gujarati"=>"$rootdir/01. Daily Murli/09. Gujarati/04. Gujarati Murli - Mp3",
    "K"=>"$rootdir/01. Daily Murli/05. Kannada/03. Kannada Murli - MP3",
    "Pun"=>"$rootdir/01. Daily Murli/11. Punjabi/04. Punjabi Murli - MP3",
    "Telugu"=>"$rootdir/01. Daily Murli/04. Telugu/03. Telugu - Murli - MP3",
    "TamilLanka"=>"$rootdir/01. Daily Murli/35. Tamil-Lanka/MP3-Tamil-Lanka",
    "Odia"=>"$rootdir/01. Daily Murli/10. Odiya/03. Odiya Murli - MP3",
  );
  ...
  $dates = array(
    ...
    date('Y-m-d', strtotime("+0 day", $curr_date_1)),
    date('Y-m-d', strtotime("-1 day", $curr_date_1)),
    date('Y-m-d', strtotime("-2 day", $curr_date_1)),
    date('Y-m-d', strtotime("-3 day", $curr_date_1)),
    date('Y-m-d', strtotime("-4 day", $curr_date_1)),
  ...
  if(isset($_POST['datelabel']) && !empty($_POST['datelabel'])) {   //Proceed button clicked
    ...
    foreach($languages_code as $lang => $code) {
      $file_to_check = $language_serverfileloc[$lang] . "/$date_dd_mm_yy-$lang.mp3";
    ...
    foreach($languages_code as $lang => $code) {
      if(isset($_POST[$code]) && !empty($_POST[$code]) && $_POST[$code] === "on" && 
        ...
        try {
          ...
          $target_file_name = "$dt_array[2].$dt_array[1]." . substr($dt_array[0], 2) . "-$lang.mp3";   //18.02.20-Deutsch.mp3
          $download_URL = "$madhuban_murli_base_url/$code/mp3/murli-" . $_POST['datelabel_hidden'] . ".mp3";
...
<html lang="en">
  <head>
    <title>Downloader - MP3 madhubanmurli.org</title>
    <meta charset="UTF-8">
  ...
  <body style="padding-top: 70px;font-family: Ubuntu;background-color: #FFEBCC">
    <?php
      $brand_name = '<i class="fa fa-download" aria-hidden="true"></i> MP3 - Madhuban ORG';
            ...
            <tbody>
              ...
              <?php
                for($i = 0; $i < count($languages); $i++) {
                  ...
                  echo '<tr>';
                    echo "<td style='width:90%;'>$date_lbl $languages[$i] MP3</td>";
                    echo '<td>';
